 ...
 * @param {{moduleName: string, apiUrl: string, version: string, singleFile: boolean, wrap: boolean, tokenPrepend: string|null, storagePrefix: string|null, modelPrepend: string|null, skipHttpGen: boolean=, skipUrlGen: boolean=, addModuleDependency: Array.<string>=, filter: {whitelist: Array.<string>, blacklist: Array.<string>}}=} options
...
module.exports = function sdkGenerator(api, options) {
  ...
  var storagePrefix = options.storagePrefix || null;
  var modelPrepend  = (options.modelPrepend) ? ('' + options.modelPrepend) : null;
  var skipHttpGen   = !!options.skipHttpGen;
  var skipUrlGen    = !!options.skipUrlGen;
  var addModuleDep  = options.addModuleDependency || null;
  ...
  var moduleFile = {
    ...
    output: mark.up(moduleMarkup, {
      url:          url,
      tokenPrepend: tokenPrepend,
      skipHttpGen:  skipHttpGen,
      skipUrlGen:   skipUrlGen,
      addModuleDep: addModuleDep || [],
...
function getNewestActionVersion(api, action) {
  if (!api.actions.versions[action]) {
    throw new Error('Could not find versions for action \'' + action + '\'. Please check your routes file, the action name is most likely incorrect.');
  }
